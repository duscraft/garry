...
const WarrantyCard: React.FC<WarrantyCardProps> = ({ warranty }) => {
  ...
  return (
    <Link 
      ...
      className="block bg-white shadow rounded-lg hover:shadow-md transition-shadow duration-200 border border-gray-100 overflow-hidden"
    >
      <div className="p-5">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 truncate" title={warranty.product_name}>
              {warranty.product_name}
            </h3>
            <p className="text-sm text-gray-500 font-medium">{warranty.brand}</p>
          </div>
        ...
        <div className="mt-4 space-y-2">
          <div className="flex items-center text-sm text-gray-500">
            <Store className="flex-shrink-0 mr-2 h-4 w-4 text-gray-400" />
            <span className="truncate">{warranty.store}</span>
          </div>
          
          <div className="flex items-center text-sm text-gray-500">
            <Tag className="flex-shrink-0 mr-2 h-4 w-4 text-gray-400" />
